...
//Code Here
let totalPopulation = populations.reduce((acc, elem) => acc + elem);
...
//Code Here
let myStrongest  = monstersInYourPocket.filter(function(elem){
    if(elem.CP > 200){
      return elem.CP;
    }
})
...
let orderTotals = orders.map(function({price, tax}){
  
  let resultTax = price * tax;
  let totalResult = price + resultTax;
  return totalResult;
